# -*- coding: utf-8 -*-
"""
串联验证：运行 `tools/crawl/cnipa_epub_search.py`（一步：爬取 + 解析，**不落盘 HTML**）。

需已安装：pip install -r tools/crawl/requirements-cnipa.txt && python -m playwright install chromium

在仓库根目录执行：

  python tests/crawl/test_cnipa_epub_chain.py

可选参数指定关键词：

  python tests/crawl/test_cnipa_epub_chain.py 批处理

无参数时本测试脚本仍传入「知识图谱」以便本地联调；**命令行直接运行** `cnipa_epub_search.py` **须自带关键词**（不设默认）。技能要求 Agent 在 Step 5 **每词一次 Bash、自行合并 JSON**（见 `prompts/disclosure/prior_art_search.md`）；本测试单次传参仅为联调。
"""
